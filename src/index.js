...
class FilterPanel extends LoadPanel {
  ...
  addFilters(panel) {
    ...
    this.addSaturationEvents(panel);
  ...
  addSaturationEvents(panel) {
    const root = panel.querySelector(".saturation");
    ...
    this.addInputEvents(this.filters.saturation);
  }

  saturation() {
    const filter = this.filters.saturation;
    const saturation = Number(filter.inputs.saturation.value);
    if (saturation === 1) {
      this.canvasContext.drawImage(this.originalCanvas, 0, 0);
    } else {
      const src = cv.imread(this.originalCanvas);
  ...
  applySeamlessEffect(src, dst, mask) {
    ...
    for (let i = 0; i < 3; i++) {
      const srcCh = srcChannels.get(i);
      const dstCh = dstChannels.get(i);
      cv.multiply(dstCh, maskFloat, dstCh);
      cv.subtract(ones, maskFloat, maskFloat);
      cv.multiply(srcCh, maskFloat, srcCh);
      cv.add(srcCh, dstCh, srcCh);
      srcCh.delete();
      dstCh.delete();
      maskFloat.convertTo(maskFloat, cv.CV_32FC1, -1.0, 1.0);
    }
    ones.delete();
    maskFloat.delete();
    dstChannels.delete();
    ...
    cv.merge(resultVec, resultFloat);
    srcChannels.delete();
    resultVec.delete();

    const result = new cv.Mat();
    resultFloat.convertTo(result, cv.CV_8UC4, 255.0);
    resultFloat.delete();
    return result;
  }
